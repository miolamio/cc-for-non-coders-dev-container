...
└── .gitignore              # course/ и skills/ — генерируются run.sh
```

Директории `course/` и `skills/` не хранятся в репозитории — `run.sh` копирует их из родительского проекта при сборке.
